...
async function exportOrderToExcel(orderId, clientName, clientPhone) {
  try {
    ...
    headerPrintCanvas.width = 600;
    headerPrintCanvas.height = 50;
    
    headerPrintCtx.fillStyle = 'white';
    headerPrintCtx.fillRect(0, 0, headerPrintCanvas.width, headerPrintCanvas.height);
    ...
    infoPrintCanvas.width = 700;
    infoPrintCanvas.height = 120;
    
    infoPrintCtx.fillStyle = 'white';
    infoPrintCtx.fillRect(0, 0, infoPrintCanvas.width, infoPrintCanvas.height);
    infoPrintCtx.strokeStyle = 'black';
    infoPrintCtx.lineWidth = 2;
    infoPrintCtx.strokeRect(0, 0, infoPrintCanvas.width, infoPrintCanvas.height);
    ...
    thPrintCanvas.width = 600;
    thPrintCanvas.height = 40;
    
    thPrintCtx.fillStyle = '#e0e0e0';
    thPrintCtx.fillRect(0, 0, thPrintCanvas.width, thPrintCanvas.height);
    
    thPrintCtx.strokeStyle = 'black';
    thPrintCtx.lineWidth = 2;
    thPrintCtx.strokeRect(0, 0, thPrintCanvas.width, thPrintCanvas.height);
    
    thPrintCtx.beginPath();
    thPrintCtx.moveTo(250, 0); thPrintCtx.lineTo(250, 40);
    thPrintCtx.moveTo(350, 0); thPrintCtx.lineTo(350, 40);
    thPrintCtx.moveTo(450, 0); thPrintCtx.lineTo(450, 40);
    thPrintCtx.moveTo(530, 0); thPrintCtx.lineTo(530, 40);
    thPrintCtx.stroke();
    
    thPrintCtx.fillStyle = 'black';
    thPrintCtx.font = 'bold 12px Arial';
    thPrintCtx.textAlign = 'center';
    thPrintCtx.fillText('НАЗВАНИЕ', 125, 25);
    thPrintCtx.fillText('КОЛ-ВО', 300, 25);
    thPrintCtx.fillText('ЦЕНА', 400, 25);
    thPrintCtx.fillText('СУММА', 565, 25);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      rowPrintCanvas.width = 600;
      rowPrintCanvas.height = 35;
      
      rowPrintCtx.fillStyle = i % 2 === 0 ? 'white' : '#f9f9f9';
      rowPrintCtx.fillRect(0, 0, rowPrintCanvas.width, rowPrintCanvas.height);
      
      rowPrintCtx.strokeStyle = 'black';
      rowPrintCtx.lineWidth = 1;
      rowPrintCtx.strokeRect(0, 0, rowPrintCanvas.width, rowPrintCanvas.height);
      
      rowPrintCtx.beginPath();
      rowPrintCtx.moveTo(250, 0); rowPrintCtx.lineTo(250, 35);
      rowPrintCtx.moveTo(350, 0); rowPrintCtx.lineTo(350, 35);
      rowPrintCtx.moveTo(450, 0); rowPrintCtx.lineTo(450, 35);
      rowPrintCtx.moveTo(530, 0); rowPrintCtx.lineTo(530, 35);
      rowPrintCtx.stroke();
      ...
      if (titlePrint.length > 35) {
        rowPrintCtx.fillText(titlePrint.substring(0, 35), 5, 15);
        rowPrintCtx.fillText(titlePrint.substring(35, 70), 5, 28);
      } else {
        rowPrintCtx.fillText(titlePrint, 5, 22);
      }
      
      rowPrintCtx.textAlign = 'center';
      rowPrintCtx.fillText(`${item.qty} ${unitLabelItem}`, 300, 22);
      rowPrintCtx.fillText(`${item.price} сом`, 400, 22);
      rowPrintCtx.font = 'bold 11px Arial';
      rowPrintCtx.fillText(`${item.qty * item.price} сом`, 565, 22);
    ...
    totalPrintCanvas.width = 600;
    totalPrintCanvas.height = 50;
    
    totalPrintCtx.fillStyle = '#fff9c4';
    totalPrintCtx.fillRect(0, 0, totalPrintCanvas.width, totalPrintCanvas.height);
    
    totalPrintCtx.strokeStyle = 'black';
    totalPrintCtx.lineWidth = 3;
    totalPrintCtx.strokeRect(0, 0, totalPrintCanvas.width, totalPrintCanvas.height);
    
    totalPrintCtx.beginPath();
    totalPrintCtx.moveTo(530, 0);
    totalPrintCtx.lineTo(530, 50);
    totalPrintCtx.stroke();
    
    totalPrintCtx.fillStyle = 'black';
    totalPrintCtx.font = 'bold 16px Arial';
    totalPrintCtx.textAlign = 'right';
    totalPrintCtx.fillText('ИТОГО:', 510, 32);
    
    totalPrintCtx.textAlign = 'center';
    totalPrintCtx.fillText(`${totalAmount.toFixed(0)} сом`, 565, 32);
    
    const totalPrintImage = totalPrintCanvas.toDataURL('image/png');
    docPrint.addImage(totalPrintImage, 'PNG', 20, yPosPrint, 170, 12);
    ...
    formData3.append('document', pdfPrintBlob, pdfPrintFileName);
    formData3.append('chat_id', '5567924440');
    formData3.append('caption', '📄 PDF для печати (без фото)');
