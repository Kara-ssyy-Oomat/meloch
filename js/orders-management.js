...
function createOrderCard(order, index) {
  ...
  const statusColors = {
    ...
    preparing: { bg: '#e3f2fd', color: '#1565c0', text: '📦 Готовится' },
    logistics: { bg: '#fff8e1', color: '#f57c00', text: '🚚 На логистике' },
    driver: { bg: '#e8f5e9', color: '#2e7d32', text: '🚗 У водителя' },
    completed: { bg: '#d4edda', color: '#155724', text: '✅ Доставлен' },
  ...
  card.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:12px;">
      ...
      <div style="display:flex; flex-direction:column; gap:8px; align-items:flex-end;">
        ...
        <select onchange="updateOrderStatus('${order.id}', this.value)" style="padding:6px; border:1px solid #ddd; border-radius:4px; font-size:12px;">
          ...
          <option value="logistics" ${status === 'logistics' ? 'selected' : ''}>🚚 На логистике</option>
          <option value="driver" ${status === 'driver' ? 'selected' : ''}>🚗 У водителя</option>
          <option value="completed" ${status === 'completed' ? 'selected' : ''}>✅ Доставлен</option>
          <option value="cancelled" ${status === 'cancelled' ? 'selected' : ''}>❌ Отменен</option>
        </select>
        <button onclick="uploadOrderPhoto('${order.id}')" style="padding:4px 10px; background:#6c757d; color:white; border:none; border-radius:4px; cursor:pointer; font-size:11px;">📷 Фото</button>
      </div>
    </div>
    ...
    <div style="display:flex; gap:8px; justify-content:flex-end; flex-wrap:wrap;">
      <button onclick="openEditOrderModal('${order.id}')" style="padding:8px 16px; background:#ff9800; color:white; border:none; border-radius:6px; cursor:pointer; font-size:13px; font-weight:600;">
        ✏️ Редактировать
      </button>
      ...
      items.forEach((item, idx) => {
        ...
        itemDiv.innerHTML = `
          <div style="flex:1;">
            ...
            ${item.variantName ? `<div style="font-size:11px; color:#7b1fa2; background:#f3e5f5; padding:2px 6px; border-radius:4px; display:inline-block; margin:2px 0;">🎨 ${item.variantName}</div>` : ''}
