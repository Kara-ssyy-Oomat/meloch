...
async function showCustomerDashboard() {
  ...
  loadingModal.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:44px;background:#f5f5f5;z-index:9500;display:flex;align-items:center;justify-content:center;';
  ...
  let customerLevel = getCustomerLevel(stats.totalSpent);
        ...
        <div style="background:#fff; padding:20px; border-bottom:1px solid #e0e0e0;">
          <div style="display:flex; align-items:center; gap:15px;">
            ...
            <div style="flex:1; cursor:pointer;" onclick="editCustomerProfile()">
              ...
              <div style="font-size:12px; color:#888; margin-top:4px;">${customerLevel.icon} ${customerLevel.name}</div>
            </div>
...
function getCustomerLevel(totalSpent) {
  if (totalSpent >= 500000) {
    return { name: 'VIP Клиент', icon: '👑', color: '#ffd700', discount: 10 };
  } else if (totalSpent >= 200000) {
    return { name: 'Золотой', icon: '🥇', color: '#ff9800', discount: 7 };
  } else if (totalSpent >= 100000) {
    return { name: 'Серебряный', icon: '🥈', color: '#9e9e9e', discount: 5 };
  } else if (totalSpent >= 50000) {
    return { name: 'Бронзовый', icon: '🥉', color: '#cd7f32', discount: 3 };
  } else {
    return { name: 'Новичок', icon: '🌟', color: '#4CAF50', discount: 0 };
  }
}
