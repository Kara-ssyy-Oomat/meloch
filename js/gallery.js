...
function showProductGallery(productId) {
  ...
  const galleryHtml = `
    <div style="position:relative; text-align:center;">
      <img id="galleryMainImg" loading="eager" crossorigin="anonymous" src="${allPhotos[0]}" alt="${product.title || ''}" style="max-width:100%; max-height:400px; border-radius:12px; margin-bottom:15px;" onerror="this.removeAttribute('crossorigin');if(!this.dataset.retry){this.dataset.retry='1';let s=this.src;this.src='';this.src=s;}">
      ...
      <div id="galleryThumbnails" style="display:flex; gap:8px; justify-content:center; flex-wrap:wrap; margin-top:10px;">
        ${allPhotos.map((url, idx) => `
          <img loading="lazy" crossorigin="anonymous" src="${url}" onclick="galleryGoTo(${idx})" style="width:50px; height:50px; object-fit:cover; border-radius:6px; cursor:pointer; border:${idx === 0 ? '3px solid #ff6b35' : '2px solid #ddd'};" class="gallery-thumb-item" data-index="${idx}" onerror="this.removeAttribute('crossorigin');this.src=this.src;">
        `).join('')}
      </div>
