...
// ВРЕМЕННО ОТКЛЮЧЕНО - прокрутка не блокируется
// ===========================================
let savedScrollPosition = 0;
let scrollLockCount = 0; // Счётчик активных модальных окон

function lockPageScroll() {
  // ВРЕМЕННО ОТКЛЮЧЕНО - не блокируем прокрутку
  console.log('lockPageScroll вызван, но отключён');
  return;
  /*
  scrollLockCount++;
...
function unlockPageScroll() {
  // ВРЕМЕННО ОТКЛЮЧЕНО
  console.log('unlockPageScroll вызван');
  forceUnlockScroll();
  return;
  /*
...
function forceUnlockScroll() {
  ...
  // НЕ восстанавливаем позицию - это может вызвать проблемы
...
// Запускаем проверку периодически (для iOS)
setInterval(checkAndRestoreScroll, 3000);

// Также проверяем при касании экрана
document.addEventListener('touchstart', function() {
  setTimeout(checkAndRestoreScroll, 500);
}, { passive: true });
