...
async function sendOrderAsExcel(name, phone, address, driverName, driverPhone, cartItems, total, time) {
  try {
    ...
    const headerImage = headerCanvas.toDataURL('image/png');
    doc.addImage(headerImage, 'PNG', 20, yPosition, 170, 12);
    ...
    yPosition += infoImageHeight + 3;
    
    // Заголовок таблицы БЕЗ колонки фото
    const tableHeaderCanvas = document.createElement('canvas');
    const thCtx = tableHeaderCanvas.getContext('2d');
    tableHeaderCanvas.width = 600;
    tableHeaderCanvas.height = 40;
    ...
    // Вертикальные линии (колонки: Название | Кол-во | Ед/кор | Всего | Цена | Сумма)
    thCtx.beginPath();
    thCtx.moveTo(200, 0); thCtx.lineTo(200, 40);
    thCtx.moveTo(300, 0); thCtx.lineTo(300, 40);
    thCtx.moveTo(380, 0); thCtx.lineTo(380, 40);
    thCtx.moveTo(450, 0); thCtx.lineTo(450, 40);
    thCtx.moveTo(520, 0); thCtx.lineTo(520, 40);
    thCtx.stroke();
    
    thCtx.fillStyle = 'black';
    thCtx.font = 'bold 12px Arial';
    thCtx.textAlign = 'center';
    thCtx.fillText('НАЗВАНИЕ', 100, 25);
    thCtx.fillText('КОЛ-ВО', 250, 25);
    thCtx.fillText('ЕД/КОР', 340, 25);
    thCtx.fillText('ВСЕГО', 415, 25);
    thCtx.fillText('ЦЕНА', 485, 25);
    thCtx.fillText('СУММА', 560, 25);
    ...
    for (let i = 0; i < cartItems.length; i++) {
      ...
      // Вертикальные линии
      rowCtx.beginPath();
      rowCtx.moveTo(200, 0); rowCtx.lineTo(200, 35);
      rowCtx.moveTo(300, 0); rowCtx.lineTo(300, 35);
      rowCtx.moveTo(380, 0); rowCtx.lineTo(380, 35);
      rowCtx.moveTo(450, 0); rowCtx.lineTo(450, 35);
      rowCtx.moveTo(520, 0); rowCtx.lineTo(520, 35);
      rowCtx.stroke();
      ...
      // Название (слева)
      rowCtx.textAlign = 'left';
      if (titleWithVariant.length > 28) {
        rowCtx.fillText(titleWithVariant.substring(0, 28), 5, 15);
        rowCtx.fillText(titleWithVariant.substring(28, 56), 5, 28);
      } else {
        rowCtx.fillText(titleWithVariant, 5, 22);
      }
      
      rowCtx.textAlign = 'center';
      rowCtx.fillText(boxText, 250, 22);
      rowCtx.fillText(`${unitsPerBox} ${unitLabel}`, 340, 22);
      rowCtx.fillText(`${item.qty} ${unitLabel}`, 415, 22);
      rowCtx.fillText(`${pricePerUnit}`, 485, 22);
      rowCtx.font = 'bold 11px Arial';
      rowCtx.fillText(`${item.qty * item.price}`, 560, 22);
    ...
    const totalCanvas = document.createElement('canvas');
    const totalCtx = totalCanvas.getContext('2d');
    totalCanvas.width = 600;
    totalCanvas.height = 50;
    ...
    totalCtx.beginPath();
    totalCtx.moveTo(520, 0);
    totalCtx.lineTo(520, 50);
    totalCtx.stroke();
    
    totalCtx.fillStyle = 'black';
    totalCtx.font = 'bold 16px Arial';
    totalCtx.textAlign = 'right';
    totalCtx.fillText('ИТОГО:', 500, 32);
    
    totalCtx.textAlign = 'center';
    totalCtx.fillText(`${total} сом`, 560, 32);
    
    const totalImage = totalCanvas.toDataURL('image/png');
    doc.addImage(totalImage, 'PNG', 20, yPosition, 170, 12);
