...
async function sendOrderAsExcel(name, phone, address, driverName, driverPhone, cartItems, total, time) {
  try {
    ...
    thCtx.beginPath();
    thCtx.moveTo(250, 0); thCtx.lineTo(250, 40);
    thCtx.moveTo(370, 0); thCtx.lineTo(370, 40);
    thCtx.moveTo(480, 0); thCtx.lineTo(480, 40);
    thCtx.stroke();
    
    thCtx.fillStyle = 'black';
    thCtx.font = 'bold 12px Arial';
    thCtx.textAlign = 'center';
    thCtx.fillText('НАЗВАНИЕ', 125, 25);
    thCtx.fillText('КОЛ-ВО', 310, 25);
    thCtx.fillText('ЦЕНА', 425, 25);
    thCtx.fillText('СУММА', 540, 25);
    ...
    for (let i = 0; i < cartItems.length; i++) {
      ...
      rowCtx.beginPath();
      rowCtx.moveTo(250, 0); rowCtx.lineTo(250, 35);
      rowCtx.moveTo(370, 0); rowCtx.lineTo(370, 35);
      rowCtx.moveTo(480, 0); rowCtx.lineTo(480, 35);
      rowCtx.stroke();
      ...
      // Название (слева)
      rowCtx.textAlign = 'left';
      if (titleWithVariant.length > 35) {
        rowCtx.fillText(titleWithVariant.substring(0, 35), 5, 15);
        rowCtx.fillText(titleWithVariant.substring(35, 70), 5, 28);
      } else {
        rowCtx.fillText(titleWithVariant, 5, 22);
      }
      
      rowCtx.textAlign = 'center';
      rowCtx.fillText(boxText, 310, 22);
      rowCtx.fillText(`${pricePerUnit} сом`, 425, 22);
      rowCtx.font = 'bold 11px Arial';
      rowCtx.fillText(`${item.qty * item.price} сом`, 540, 22);
