...
async function sendOrderAsPDF(name, phone, address, driverName, driverPhone, cartItems, total, time) {
  try {
    ...
    tableHeaderCanvas.width = 550 * scale;
    tableHeaderCanvas.height = 50 * scale;
    ...
    for (let i = 0; i < cartItems.length; i++) {
      ...
      // Создаем строку таблицы с динамической шириной и высоким разрешением
      ...
      rowCanvas.width = totalWidth * scale;
...
async function sendOrderAsExcel(name, phone, address, driverName, driverPhone, cartItems, total, time) {
  try {
    ...
    for (let i = 0; i < cartItems.length; i++) {
      ...
      rowCtx.fillText(`${pricePerUnit} сом`, 450, 22);
