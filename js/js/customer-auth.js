...
async function loginCustomer(phone, password) {
  try {
    ...
    // Привязываем chatClientId к профилю клиента (для рассылки)
    if (typeof clientId !== 'undefined' && clientId) {
      try {
        await db.collection('customers').doc(customerDoc.id).update({ chatClientId: clientId });
      } catch(e) {}
    }
...
async function registerCustomer(data) {
  try {
    ...
    const customerRef = await db.collection('customers').add({
      ...
      chatClientId: (typeof clientId !== 'undefined' && clientId) ? clientId : null
    });
